...
@Component({
  selector: 'app-consultations-rejected-admin',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './consultations-rejected-admin.component.html',
  styleUrls: ['./consultations-rejected-admin.component.css']
})
export class ConsultationsRejectedAdminComponent implements OnInit {
  ...
  loadRejectedByAdminConsultations(): void {
    ...
    this.consultationService.getConsultationsByStatus('refuse').subscribe({
      next: (data: any) => {
        ...
        // Filtrer pour ne garder que celles refusées par l'admin (approuve_admin = false)
        this.consultations = consultations.filter((consultation: any) => 
          consultation.approuve_admin === false || consultation.approuve_admin === 0
        );
        this.applyFilters();
        this.loading = false;
      },
