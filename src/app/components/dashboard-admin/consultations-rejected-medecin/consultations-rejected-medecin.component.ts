...
@Component({
  selector: 'app-consultations-rejected-medecin',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './consultations-rejected-medecin.component.html',
  styleUrls: ['./consultations-rejected-medecin.component.css']
})
export class ConsultationsRejectedMedecinComponent implements OnInit {
  ...
  /**
   * Charge les consultations refusées (par admin ou médecin)
   */
  loadRejectedByMedecinConsultations(): void {
    this.loading = true;
    this.consultationService.getConsultationsByStatus('refuse').subscribe({
      next: (data: any) => {
        // Vérifier si data est un tableau ou un objet avec consultations
        const consultations = Array.isArray(data) ? data : data.consultations || [];
        // Filtrer pour ne garder que celles refusées par le médecin (approuve_admin = true)
        this.consultations = consultations.filter((consultation: any) => 
          consultation.approuve_admin === true || consultation.approuve_admin === 1
        );
        this.applyFilters();
        this.loading = false;
      },
